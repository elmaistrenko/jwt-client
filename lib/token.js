...
const actualFormatted = async ({
    ...
    result = token => {
        const tokenTypePart = token[tokenType] || defaultTokenType;
        if (tokenTypePart)
            tokenTypePart += ' ';
    ...
    if (
        ...
        token.creationTs &&
        token.creationTs + token[accessExpiresIn]*1000 > +new Date()
    ) {
        ...
        token.creationTs &&
        token.creationTs + token[refreshExpiresIn]*1000 > +new Date()
    ) {
